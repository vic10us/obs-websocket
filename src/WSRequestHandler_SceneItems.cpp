 ...
 * @since 4.7.0
 */
RpcResponse WSRequestHandler::HandleSetSceneItemColor(const RpcRequest& request)
{
	if (!request.hasObject("item") || !request.hasString("color")) {
		return request.failed("missing or invalid request parameters");
	}
	...
	OBSDataAutoRelease itemInfo = obs_data_get_obj(request.parameters(), "item");
	OBSSceneItemAutoRelease sceneItem = Utils::GetSceneItemFromItem(scene, itemInfo);
	if (!sceneItem) {
		return request.failed("item with id/name combination not found in specified scene");
	}

	QString color = obs_data_get_string(request.parameters(), "color");
